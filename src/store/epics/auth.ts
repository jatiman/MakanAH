...
export class AuthEpic {
    constructor(private afauth: AngularFireAuth, private http: Http, public restProvider: RestProvider) {}
    ...
    login = (actions$: ActionsObservable<any>) => {
        ...
        .switchMap(({payload, navCtrl}) => {
            ...
            .switchMap((responce) => {
                ...
                return this.http.post(url, payload, {headers: headers})
                .switchMap(res => {
                    this.restProvider.rslt = res;
                    navCtrl();
                    return Observable.of({type: LOGIN_SUCCESS, payload: payload, res});
                })
                .catch((error) => {
